<?php
    try {
        ...
        if($fecha_actual == $fecha && $horaActual >= $hora_inicio){
            echo "<div class='notification-error' id='myNotification'>La hora de inicio debe ser mayor a la hora actual.</div><br>";
        ...
        if (!empty($fechas_bloquedas)) {
            echo "<div class='notification-error' id='myNotification'>Lo sentimos, esa fecha no esta disponible</div><br>";
        } else {
            ...
            if ($duracion > 240 || $duracion < 30) {
                echo "<div class='notification-error' id='myNotification'>La reserva no puede durar más de 4 horas ni menos de 30 minutos.</div><br>";
            } else {
                $sql_mesas = "SELECT id_mesa FROM mesas 
                WHERE capacidad >= :numero_personas 
                AND id_mesa NOT IN (SELECT DISTINCT id_mesa FROM reservas WHERE fecha = :fecha AND NOT (hora_fin <= :hora_inicio OR hora_inicio >= :hora_fin) AND id_mesa!= :id_mesa)
                AND estado = 'Disponible' 
                ORDER BY capacidad ASC
                LIMIT 1";
                $mesas= $base->prepare($sql_mesas);
                $mesas->bindValue(":id_mesa", $id_mesa);
                ...
                if (!empty($mesas_disponibles)) {
                    ...
                    $sentenciaSQL = "UPDATE  RESERVAS SET id_usuario= :id_usuario, id_mesa = :id_mesa, numero_personas = :numero_personas, hora_inicio = :hora_inicio, hora_fin = :hora_fin, fecha = :fecha WHERE id_reserva= :id_reserva";
                    ...
                    $resultado->execute();
                    echo "<div class='notification' id='myNotification'>La reserva se edito correctamente.</div><br>";
                } else {
                    echo "<div class='notification-error' id='myNotification'>Lo sentimos, no hay mesas disponibles para esa cantidad de personas en ese horario.</div><br>";
                }
            }
        }
    }
